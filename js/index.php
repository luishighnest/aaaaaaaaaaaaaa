...
<body class="dashboard-body">
  ...
  <script>
    ...
    // Inizializza i canali preferiti del profilo attivo
    ...
    // Categoria "I Miei Preferiti"
    const aFav = document.createElement('a');
    aFav.href = '#';
    aFav.dataset.color = '#ffc107'; // Colore Oro
    aFav.className = 'dash-cat-item' + ('favorites' === currentCat ? ' active' : '');
    aFav.id = 'cat-favorites-btn';
    aFav.innerHTML = `<i class="ph ph-star" style="color: ${'favorites' === currentCat ? 'var(--text-primary)' : '#ffc107'}"></i> I Miei Preferiti <span class="dash-cat-count" id="fav-count">${favorites.length}</span>`;
    aFav.onclick = (e) => {
      e.preventDefault();
    ...
    function renderChannels(catKey, searchQuery = '') {
      ...
      for (let i = 0; i < filtered.length; i++) {
        ...
        html += `<div class="dash-ch-col-info">`;
        html += `<div class="dash-ch-name" style="display:flex;align-items:center;justify-content:space-between;width:100%;gap:6px;">`;
        html += `<div style="display:flex;align-items:center;gap:6px;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">`;
        if (isPlaying) html += `<span class="dash-ch-live-badge">LIVE</span>`;
        html += `<span class="dash-ch-title-text" style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${ch.name}</span>`;
        html += `</div>`;
        const isFav = favorites.includes(ch.id);
        html += `<span class="ch-row-fav-btn" onclick="event.stopPropagation(); event.preventDefault(); toggleFavorite(${ch.id});" style="cursor:pointer;color:${isFav ? '#ffc107' : 'rgba(255,255,255,0.1)'};flex-shrink:0;padding:2px;transition:all 0.2s;" title="${isFav ? 'Rimuovi dai preferiti' : 'Aggiungi ai preferiti'}"><i class="${isFav ? 'ph-fill ph-star' : 'ph ph-star'}"></i></span>`;
        html += `</div>`;
        html += `<div class="dash-ch-cat" style="color:${catColor}">${cMeta ? cMeta.label : ch.cat}</div>${searchMatchHtml}</div>`;
    ...
    function selectChannel(ch) {
      ...
      if (favBtn) {
        ...
        if (isFav) {
          ...
          if (favIcon) favIcon.className = "ph-fill ph-star";
        } else {
          favBtn.classList.remove('is-favorite');
          favBtn.title = "Aggiungi ai preferiti";
          if (favIcon) favIcon.className = "ph ph-star";
        }
        favBtn.onclick = (e) => {
          e.preventDefault();
    ...
    async function toggleFavorite(channelId) {
      try {
        ...
        if (data.success && Array.isArray(data.favorites)) {
          // Aggiorna l'array globale dei preferiti
          favorites = data.favorites;
          window.favorites = favorites;
          
          // 1. Aggiorna il contatore della categoria in tempo reale
          const favCountEl = document.getElementById('fav-count');
          if (favCountEl) favCountEl.textContent = favorites.length;
          
          // 2. Aggiorna lo stato visivo del bottone nel Player se riguarda il canale attivo
          ...
          // 3. Se siamo nella categoria dei preferiti ed il canale è stato rimosso, rinfreschiamo per farlo sparire fluidamente
          if (currentCat === 'favorites') {
            renderChannels('favorites');
          } else {
            // Altrimenti aggiorniamo solo le stelle dei canali visualizzati a schermo
            document.querySelectorAll('.dash-ch-row').forEach(row => {
              const rowIdAttr = row.getAttribute('href');
              if (rowIdAttr) {
                const rowId = parseInt(rowIdAttr.replace('?id=', ''));
                const starBtn = row.querySelector('.ch-row-fav-btn');
                if (starBtn && !isNaN(rowId)) {
                  const isFav = favorites.includes(rowId);
                  starBtn.style.color = isFav ? '#ffc107' : 'rgba(255,255,255,0.1)';
                  const starIcon = starBtn.querySelector('i');
                  if (starIcon) {
                    starIcon.className = isFav ? 'ph-fill ph-star' : 'ph ph-star';
                  }
                }
              }
            });
