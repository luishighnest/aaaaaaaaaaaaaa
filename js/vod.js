...
async function togglePlayerPlaylistPanel() {
    ...
    // Chiudi pannello info se aperto
    ...
    // Se non è una serie TV, mostra messaggio
    if (!ctx || ctx.type !== 'tv') {
        panel.innerHTML = `
    ...
    try {
        const showResp = await fetch(`${BASE_URL}/tv/${tmdbId}?api_key=${API_KEY}&language=it-IT`);
        ...
        const renderEpisodes = async (seasonNum) => {
            ...
            try {
                const epResp = await fetch(`${BASE_URL}/tv/${tmdbId}/season/${seasonNum}?api_key=${API_KEY}&language=it-IT`);
                const epData = await epResp.json();
                const episodes = epData.episodes || [];

                episodesContainer.innerHTML = '';
                episodes.forEach(ep => {
                    const isActive = parseInt(ep.episode_number, 10) === parseInt(currentEpisode, 10)
                                  && parseInt(seasonNum, 10) === parseInt(currentSeason, 10);
                    const progress = getPreviousProgress(tmdbId, 'tv', seasonNum, ep.episode_number);
                    ...
                    epEl.addEventListener('click', () => {
                        ...
                        setTimeout(() => playShowEpisode(tmdbId, seasonNum, ep.episode_number), 200);
                    });
                    episodesContainer.appendChild(epEl);
                });

                // Scroll all'episodio attivo
                const activeEp = episodesContainer.querySelector('.vod-playlist-episode.active');
                if (activeEp) {
                    setTimeout(() => activeEp.scrollIntoView({ behavior: 'smooth', block: 'center' }), 120);
                }
            } catch(err) {
